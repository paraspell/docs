...
const builder = Builder(/*client | builder_config |ws_url | [ws_url, ws_url,..] - Optional*/)
      ...
      .to(TChain /*,customParaId - optional*/ | Location object /*Only works for PolkadotXCM pallet*/) //'AssetHubPolkadot' | 'Hydration' | 'BifrostPolkadot' | 'Polkadot' |  ... https://paraspell.github.io/docs/xcm-sdk/asset-package.html#import-chains-as-types
...
```ts
.ahAddress(ahAddress) // - OPTIONAL - used when origin is EVM CHAIN and XCM goes through AssetHub (Multihop transfer where we are unable to convert Key20 to ID32 address
...
**Turned on by default.** The following setting will abstract decimals from the .currency builder functionality.
...
The following setting will perform a dryrun bypass for each call under the hood. This will ensure the XCM Format is correct and will prevent the SDK from opening the wallet if the dryrun bypass does not pass, meaning that the XCM Format is incorrect.
...
The following example has every option enabled.
```ts
const builder = await Builder({
  development: true, // Optional: Enforces overrides for all chains used
  abstractDecimals: true, // Abstracts decimals from amount - so 1 in amount for DOT equals 10_000_000_000
...
::: details Transferring 1 USDT transfer from Hydration to AssetHubPolkadot 
```ts
...
**Builder example**
::: details Transferring 1 DOT from one account to another on Hydration
...
const builder = Builder(/*client | builder_config | ws_url | [ws_url, ws_url,..] - Optional*/)
      ...
      .recipient(address)
      .transact(hex, /* originType, TWeight - Optional */) // Refer to transact spec below
...
**Builder example**
::: details Transferring 1 DOT and performing transact which transfers all native currency on Bifrost
...
## Swap
If you [installed Swap package](https://paraspell.github.io/docs/xcm-sdk/getting-started.html#install-swap-extension), you can create Swap XCMs. Doing so, you can send one asset and receive another on the destination. The caveat to using this method is that some transfers are one-click (like the rest of XCM transfers) while others are two-click (majority: sending from origin to exchange is one signature, and then exchange + transfer to destination is a second signature). You need a specialized method to build these: `.buildAll()`.
...
const result = hasDryRunSupport(TChain) // 'AssetHubPolkadot' | 'Hydration' | 'BifrostPolkadot' | 'Polkadot' |  ... https://paraspell.github.io/docs/xcm-sdk/asset-package.html#import-chains-as-types
```
:::
...
## Ecosystem bridges
List of available bridges in the XCM SDK. Implementing cross-ecosystem asset transfers has never been easier.
...
Available as a [Snowbridge extension](https://paraspell.github.io/docs/xcm-sdk/getting-started.html#install-snowbridge-extension) in all XCM SDK versions. **Uses the viem package.**
...
## Batch calls
You can batch XCM calls and execute them all within a single call. All three scenarios (Para->Para, Para->Relay, Relay->Para) can be used and combined.
```ts
await Builder(/*CHAIN api/ws_url_string - optional*/)
      ...
      .to(Tchain2) // 'AssetHubPolkadot' | 'Hydration' | 'BifrostPolkadot' | 'Polkadot' |  ... https://paraspell.github.io/docs/xcm-sdk/asset-package.html#import-chains-as-types
      .currency(CURRENCY_SPEC) // Refer to currency spec options below
      .recipient(address | Location object)
      .addToBatch()

      .from(TSubstrateChain) // Ensure that origin CHAIN is the same in all batched XCM Calls.
      .to(TChain3) // 'AssetHubPolkadot' | 'Hydration' | 'BifrostPolkadot' | 'Polkadot' |  ... https://paraspell.github.io/docs/xcm-sdk/asset-package.html#import-chains-as-types
      .currency(CURRENCY_SPEC) // Refer to currency spec options below
      .recipient(address | Location object)
      .addToBatch()
      .buildBatch({ 
...
The SDK features the ability to add a custom chain and/or custom assets simply by adding its config to the Builder.
        ...
        assets: [
          {
            symbol: 'USDC',
            decimals: 6,
            existentialDeposit: 1000000000, //Needs to be in plancks,
            location: {
              parents: 1,
...
The following functionality allows you to convert any SS58 address to a Parachain-specific address. The function uses [TChain](https://paraspell.github.io/docs/xcm-sdk/asset-package.html#import-chains-as-types) types.
...
## Query asset reserve
The following query allows you to query the asset reserve for a specific asset on a specific chain.
...
There are 6 options for types you can choose based on your preference
...
There are 6 options for constants you can choose based on your preference
