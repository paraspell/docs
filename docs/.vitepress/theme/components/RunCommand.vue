...
const command = computed(() => {
  const suffix = props.args ? ` ${props.args}` : ''
  switch (packageManager.value) {
    case 'yarn':
      return `yarn dlx ${props.pkg}${suffix}`
    case 'npm':
      return `npx ${props.pkg}${suffix}`
    case 'bun':
      return `bunx ${props.pkg}${suffix}`
    default:
      return `pnpm dlx ${props.pkg}${suffix}`
  }
})
