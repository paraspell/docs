...
#### 9. Exchange parameter now takes standard TSubstrateChain

##### What changed
- .exchange() parameter in Swap package now takes standard `TSubstrateChain` parameter, removing `Dex` suffix.
...
##### Migration
Use standard TSubstrateChain type:
```ts
.exchange("AssetHubPolkadot")
```
