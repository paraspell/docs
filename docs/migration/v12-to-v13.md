...
### 🛠️ Specific XCM API breaking changes

The XCM API has now moved under the paraspell.xyz domain, so instead of `https://api.lightspell.xyz/v5/assets/Astar` you now use `https://api.paraspell.xyz/v1/assets/Astar`.
...
- Use [**XCM fee queries**](https://paraspell.github.io/docs/xcm-sdk/xcm-utils.html#xcm-fee-origin-and-dest) instead
...
Available as [swap endpoint](https://paraspell.github.io/docs/xcm-api/xcm-sdk-functionality.html#swap) in API.

##### Migration
- Replace `/router` endpoint with [swap endpoint](https://paraspell.github.io/docs/xcm-api/xcm-sdk-functionality.html#swap)
...
These are deprecated in favor of `findAssetInfo`.
...
##### After (v13)
```ts
// If not given, nothing changes (unless destination is Ethereum and you are searching by Asset symbol, make sure to provide it in that case because you can get a duplicate asset error)
findAssetInfo(chain, CURRENCY_SPEC, destination?)
```
...
#### Behavior
- Returns an **array of transaction hexes**
...
```ts
// v13
signAndSubmit()     // Used in one-click scenarios & basic transfers when a signer is provided to sender()
signAndSubmitAll()  // Used in two-click swap scenarios when a signer is provided to sender()
```
