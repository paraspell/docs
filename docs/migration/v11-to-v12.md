...
v12 introduces the following major changes:
...
### What changed
In earlier versions, the SDK exposed `TNode` types to represent chain identifiers.  
During v11, it was replaced by **`TChain`** types and marked as deprecated.  
In v12, **`TNode` types have been fully removed**. All available `TChain` types can be found listed [here](https://paraspell.github.io/docs/xcm-sdk/asset-package.html#import-chains-as-types).
...
### What changed
In early versions of our XCM Fee queries, we introduced a currency identifier in the form of a symbol. As features grew more robust, a full asset object became necessary, mainly because the primary selection method is by `location`. Some chains have multiple assets with similar or identical symbols, the only difference between them being capital or lowercase letters. The `currency: symbol` field grew obsolete after the full asset object was introduced. Keeping it caused duplication and confusion.
...
Previously, we only had AssetHub and BridgeHub as possible hop chains in XCM Fee and Dry-run queries. As the features grew more robust, we added a hop array that is more generic and could house any parachain as a hop. We removed the AssetHub and BridgeHub objects because they would occur redundantly, both on their own and also in the hops array, which caused unnecessary confusion.
...
These were already undocumented, and in v12 they have been **fully removed**. The unified `getBalance` query handles both cases automatically and is preferred.

Since they have been removed, we took the opportunity to make the balance query name more intuitive. It is now renamed from `getAssetBalance` to `getBalance`.

Additionally, the `currency` parameter in the balance query is now **optional**.  
If omitted, the query defaults to the selection of the chain’s native asset.
...
✅ **Action Required:**  
- Replace all usages of the `getNativeAssetBalance` or `getForeignAssetBalance` queries with the `getBalance` query.  
- Rename `getAssetBalance` to `getBalance`.
...
The SDKs and Router have migrated to **ESM-only**.  
...
We have added support for CommonJS in XCM Analyser (as that package is mostly vanilla).
...
- If your project uses CommonJS and had to adapt to ESM because of the XCM Analyser library, you can convert back to CommonJS, as Analyser now also supports CommonJS.
...
Previously, our asset registry divided assets under each chain into two separate arrays:
...
In v12, asset definitions are unified under a **single `assets` array**, where native assets are identified by an `isNative: true` flag. This allowed us to significantly improve code quality, save package space, and optimize certain types and functions that needed to pick between the two arrays.
...
Use the `assets` array exclusively and filter by `isNative` if you are searching for the native asset within the chain.
...
This change was done to optimize the code and to unify the selection of MYTH.
...
For a more detailed overview, we list the v12 [pull request](https://github.com/paraspell/xcm-tools/pull/1516).